document.addEventListener('DOMContentLoaded', async () => {
    ...
    const displayData = () => {
        ...
        currentData.forEach(item => {
            ...
            tourismTitle.textContent = item.nama_partner;
            tourismTitle.addEventListener('click', () => {
                window.location.href = `detail-rental.html?id=${item.id}`;
            });

            const tourismImage = document.createElement('img');
            tourismImage.src = `http://localhost:3000/uploads/${item.foto_rental}`;
            tourismImage.alt = item.nama_partner;

            const tourismDescription = document.createElement('p');
            tourismDescription.textContent = item.deskripsi_rental;

            const tourismJenis = document.createElement('p');
            tourismJenis.textContent = `Jenis: ${item.jenis}`;

            tourismItem.appendChild(tourismTitle);
            tourismItem.appendChild(tourismImage);
            tourismItem.appendChild(tourismJenis);
